...
function smartChatUpgrade() {
    ...
    bar.id = 'smartChatBar';
    bar.style.cssText = 'display:flex;gap:4px;align-items:center;flex-wrap:nowrap;overflow:visible;';  
	    bar.innerHTML =
        '<button id="scPhotoBtn" title="Send photo" style="width:36px;height:36px;min-width:36px;border:none;border-radius:50%;background:#7c3aed;color:white;font-size:15px;cursor:pointer;flex-shrink:0;align-self:flex-end;margin-bottom:2px;">📷</button>' +
        '<button id="scMicBtn" title="Hold to record voice" style="width:36px;height:36px;min-width:36px;border:none;border-radius:50%;background:#ef4444;color:white;font-size:15px;cursor:pointer;flex-shrink:0;align-self:flex-end;margin-bottom:2px;">🎤</button>' +
        '<div style="flex:1;min-width:0;position:relative;">' +
            '<textarea id="scTextInput" rows="1" placeholder="Message..." style="width:100%;box-sizing:border-box;padding:9px 48px 9px 14px;border:1px solid #cbd5e1;border-radius:18px;font-size:14px;outline:none;resize:none;overflow-y:auto;line-height:1.4;max-height:105px;background:white;color:#1e293b;font-family:inherit;display:block;"></textarea>' +
            '<button id="scSendBtn" title="Send" style="position:absolute;right:4px;bottom:4px;width:32px;height:32px;min-width:32px;border:none;border-radius:50%;background:#2563eb;color:white;font-size:13px;font-weight:bold;cursor:pointer;">➤</button>' +
        '</div>';
    inputArea.appendChild(bar);

       document.getElementById('scSendBtn').onclick = function() {
        var ti = document.getElementById('scTextInput');
    ...
    var ti = document.getElementById('scTextInput');
    ti.addEventListener('input', autoGrowTextArea);
    ti.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && !e.shiftKey) {
            e.preventDefault();
            document.getElementById('scSendBtn').click();
            ti.style.height = '36px'; // reset after send
        }
        // Shift+Enter = new line (natural typing!)
    });
...
// 📈 Auto-grow: input expands upward as you type (max 5 lines)
function autoGrowTextArea() {
    var input = document.getElementById('scTextInput');
    if (!input) return;
    input.style.height = 'auto';
    var newHeight = Math.min(input.scrollHeight, 105);
    input.style.height = newHeight + 'px';
    input.style.overflowY = input.scrollHeight > 105 ? 'auto' : 'hidden';
}
// ================================================================
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
        ...
        if (label) {
            __recTimer = setInterval(function() {
                t++;
                label.value = '🔴 ' + t + 's';
                label.style.color = '#ef4444';                if (t >= 60) __stopRecording(true);
            }, 1000);
        }
    }).catch(function() {
